...
public class DeviceServiceImpl implements DeviceService {
    ...
    public Response assignDevice(DeviceDto device) {
        System.out.println("-----------  "+entityManager.createQuery("select pd from PatientDevice pd left join fetch pd.activitiesSteps steps where pd.id = steps.patientDevice.id and pd.medicalFileId = :medicalFileId and steps.dateTime between :date1 and :date2 group by steps.dateTime")
