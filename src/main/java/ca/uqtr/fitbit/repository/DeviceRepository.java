...
public interface DeviceRepository extends CrudRepository<Device, UUID> {
    ...
    @Query("select d from Device d left join fetch d.patientDevices pd on d.id = pd.device.id where pd.medicalFileId = :patientDevice AND pd.returnedAt IS NULL ORDER BY pd.id DESC")
